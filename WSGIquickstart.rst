...
.. note:: Do not use ``--http`` when you have a frontend webserver or you are doing some form of benchmark, use ``--http-socket``. Because if you use ``--http`` then uWSGI natively speak the http protocol. It makes a frontend webserver receving wrong protocl and making ``502 bad gateway error``. Continue reading the quickstart to understand why.
